...
2. Создать виртуальную машину на яндекс клауд
3. Подключиться к ВМ через ssh
   ```
   ssh fenya@<public_vm_ip>
   ```
4. Установить на ВМ jre-17, maven, git
5. Склонировать github-репозиторий проекта на ВМ. git clone (считаем, что ключи для github у вас настроены)
   ```
   git clone git@github.com:Fenya2/novocolcovo-bot.git
   ```
6. Собрать проект на ВМ через maven (предварительно добавив в инструкцию по сборке)
   ```
   cd ./novocolcovo-bot
   mvn package
   ```
7. Создать файл novocolcovo-bot в system manager, прописав команду с переменными окружения с запуском собранного jar-файла на ВМ java.  
   see https://linuxhandbook.com/create-systemd-services/  
   команда для запуска, прописанная в файле конфига сервиса будет иметь вид:
   ```
   env TG_BOT_TOKEN=<your tg token> VK_BOT_TOKEN=<your vk token> java -jar /home/<username>/novocolcovo-bot/target/<compiled-executable-jar>
   ```
8. Запустить сервис.
   ```
   sudo systemctl start novocolcovo-bot
   ```
